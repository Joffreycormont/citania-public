...
class UserAuthenticator extends AbstractFormLoginAuthenticator implements PasswordAuthenticatedInterface
{
    ...
    public function supports(Request $request)
    {
        ...
        if($request->attributes->get('_route') === 'home'){
            return $request->attributes->get('_route') === $request->attributes->get('_route')
            && $request->isMethod('POST');
        }elseif($request->attributes->get('_route') === 'user_register'){
            return $request->attributes->get('_route') === $request->attributes->get('_route')
            && $request->isMethod('POST');
        }elseif($request->attributes->get('_route') === 'CGU'){
            return $request->attributes->get('_route') === $request->attributes->get('_route')
            && $request->isMethod('POST');
        }elseif($request->attributes->get('_route') === 'contact'){
            return $request->attributes->get('_route') === $request->attributes->get('_route')
            && $request->isMethod('POST');
        }elseif($request->attributes->get('_route') === 'app_password'){
            return $request->attributes->get('_route') === $request->attributes->get('_route')
            && $request->isMethod('POST');
        }else{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, $providerKey)
    {
        if ($targetPath = $this->getTargetPath($request->getSession(), $providerKey)) {
            ...
            return new RedirectResponse($this->urlGenerator->generate('is_connected'));
        }
